      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div v-if="!isLoading && events.length > 0" class="mb-8 p-4 bg-blue-900/20 border border-blue-500/30 rounded-lg">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-blue-300 font-medium">AI-Generated Events</h3>
              <p class="text-blue-200/80 text-sm">These events are generated by ChatGPT based on current AI industry developments and realistic projections for Stargate Project and Cristal Intelligence.</p>
            </div>
          </div>
        </div>
        ...
        <div v-if="isLoading" class="text-center py-12">
          ...
          <p class="mt-4 text-gray-400">Loading events from ChatGPT...</p>
        </div>
...
const loadEvents = async (category?: string) => {
  ...
  try {
    ...
    if (latestResponse.success && latestResponse.events.length > 0) {
      ...
      console.log('📅 Loaded latest events from ChatGPT:', latestResponse.events.length)
    } else {
...
const refreshEvents = async () => {
  ...
  try {
    ...
    if (response.success) {
      events.value = response.events
      console.log('🔄 Refreshed events from ChatGPT:', response.events.length)
    } else {
